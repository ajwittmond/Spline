function Graph(elem,width,height){
	...
	this.snapInputToGrid = false;
	
	var setScaledGrid = function(){
		var base = 10;
		var divs = base;
		var num = base/obj.scale;
		if(num>base){
			while(divs<num)
				divs*=base;
		}else{
	...
	Object.defineProperty(this,"scaleGrid",{
		set: function(val){
			scaleGrid = val;
			if(val){
				setScaledGrid();
	...
	var drawGrid = function(){
		...
		var lstart = Math.floor(left/gridW) * gridW;
		var bstart = Math.floor(bottom/gridH) * gridH;
		
		ctx.beginPath();
		ctx.strokeStyle = "rgb(200,200,200)";
		//ctx.setLineDash([(divs/10)*(2/3),(divs/10)*(1/3)]);
		
		for(let vloc = lstart; vloc<right; vloc+=gridW){
			ctx.moveTo(vloc,bstart);
			ctx.lineTo(vloc,tp);
			ctx.stroke();
		}
		
		for(let hloc = bstart; hloc<tp; hloc+=gridH){
		...
		for(let vloc = lstart; vloc<right; vloc+=gridW){
			...
			ctx.fillText(Math.round(vloc*10000)/10000+"",vloc*obj.scale,-obj.scale*(bottom + (tp-bottom)*0.005),obj.scale * gridW)
			ctx.restore();
		}
		...
		for(let hloc = bstart; hloc<tp; hloc+=gridH){
			...
			ctx.translate(obj.x+elem.width/2,obj.y+elem.height/2)
			ctx.fillStyle= "rgb(100,100,100)"
			ctx.fillText(Math.round(hloc*10000)/10000+"",obj.scale*(left + (right-left)*0.005),-obj.scale*hloc,obj.scale * gridH)
			ctx.restore();
		}
	...
	elem.onmousedown = function(evt){
		...
		if(obj.snapInputToGrid){
			p[0] = Math.round(p[0]/gridW)*gridW;
			p[1] = Math.round(p[1]/gridH)*gridH;
		}
